package com.zhouqiang.framework.fileload;
...
class DownLoadThread extends Thread {
	...
	public void run() {
		if (isThreadFinished()) {
			Logger.d( getName() + "已经完成");
			finished = true;
			return;
		}

		Logger.d( getName() + "开始执行");
		...
		try {
			...
			con.setConnectTimeout(BaseConfig.TIMEOUT_CONNECT_FILE);
			con.setReadTimeout(BaseConfig.TIMEOUT_READ_FILE);
			...
			if (!stop) {
				finished = true;
				Logger.d( getName() + "执行成功");
			} else {
				Logger.d( getName() + "执行中断");
			}
		} catch (Exception e) {
			Logger.d( getName() + "执行失败");
			failed = true;
		} finally {
			try {
				if (bis != null)
