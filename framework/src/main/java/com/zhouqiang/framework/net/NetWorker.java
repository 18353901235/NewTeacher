...
public class NetWorker extends BaseObject {
    ...
    public NetWorker(Context mContext) {
        Looper looper;
...
//加密
    ...
    private class NetThread extends Thread {
        private ArrayList<NetTask> tasks = new ArrayList<NetTask>();
        private boolean isRun = true;

        NetThread(NetTask task) {
            tasks.add(task);
            setName("网络线程(" + getName() + ")");
        }

        void addTask(NetTask task) {
        ...
        public void run() {
            ...
            while (isRun) {
                ...
                NetTask currTask = tasks.get(0);
                TR<NetTask, Object> tr = new TR<NetTask, Object>();
                tr.setTask(currTask);
                beforeDoTask(tr);
        ...
        private void beforeDoTask(TR<NetTask, Object> result) {
        ...
        // 执行网络请求任务
        private void doTask(TR<NetTask, Object> result, Message mess) {
            final NetTask task = result.getTask();
            //    log_d("Do task !!!Try " + (task.getTryTimes() + 1));
            log_d("The Task Description: " + task.getDescription() + "--->" + task.getPath() + "?" + task.getParams().toString());
            try {
                Object object;
                if (task.getFiles() == null) {
                    String json = HttpUtil.sendPOSTForString(
                            task.getPath(), task.getParams(),
                            BaseConfig.ENCODING);
                    object = task.parse(json);
                } else {


                    String json = HttpUtil
                            .sendPOSTWithFilesForString(task.getPath(),
        ...
        // 失败后再试几次
        private void tryAgain(NetTask task, int type, Message mess,
                              TR<NetTask, Object> result) {
            task.setTryTimes(task.getTryTimes() + 1);
            if (task.getTryTimes() >= BaseConfig.TRYTIMES_HTTP) {
                mess.what = type;
                // mess.arg1 = task.getId();
                mess.obj = result;
                tasks.remove(task);
    ...
    private static class EventHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (listener != null) {
                ...
                TR<NetTask, Object> result = (TR<NetTask, Object>) msg.obj;
                switch (msg.what) {
                    case SUCCESS:
    ...
    public interface OnTaskExecuteListener {
         ...
         */
        void onPreExecute(NetWorker netWorker, NetTask task);

        /**
         * Runs on the UI thread after the task run.
         */
        void onPostExecute(NetWorker netWorker, NetTask task);
         ...
         */
        void onExecuteSuccess(NetWorker netWorker, NetTask task,
                              Object result);
