...
import com.zhouqiang.framework.util.FileTypeUtil;
import com.zhouqiang.framework.util.Logger;
import com.zhouqiang.framework.util.Md5Util;
import com.zhouqiang.framework.util.StreamUtil;
...
public class HttpUtil {
    ...
    private static final String BOUNDARY = "zq";//周强
    ...
    public static String sendPOSTWithFilesForString(String path,
        ...
        try {
            ...
            Logger.d("The HttpUrl is \n" + path);
            conn = (HttpURLConnection) url.openConnection();
            // 设置每次传输的流大小，可以有效防止手机因为内存不足崩溃
            // 此方法用于在预先不知道内容长度时启用没有进行内部缓冲的 HTTP 请求正文的流。
            // 设置此参数后导致请求有时成功有时失败（网上说是服务器不支持,具体原因未明）
            // conn.setChunkedStreamingMode(0);// 128K 128 * 1024
            conn.setConnectTimeout(BaseConfig.TIMEOUT_CONNECT_HTTP);
            conn.setReadTimeout(BaseConfig.TIMEOUT_READ_FILE);
            ...
            if (entry.getValue() != null) {
                if (entry.getValue().contains(",")) {
                    ...
                    for (String path : split) {
                        upFile(dos, entry.getKey(), path);
                    }
                } else {
                    upFile(dos, entry.getKey(), entry.getValue());
                }
            }
        }
    }

    private static void upFile(DataOutputStream dos, String name, String values) throws IOException {
        ...
        try {
            ...
            dos.writeBytes("Content-Disposition: form-data; " + "name=\""
                    ...
                    + "\"" + END);
            Logger.d("The file path is \n" + name + "----->" + values);
            String filetype = FileTypeUtil.getFileTypeByPath(values);// 获取文件类型
            Logger.d("The file type is " + filetype);
...
//                if (entry.getKey().equals("content1")) {
//                    dos.write((" " + "deddedededeedede").getBytes());
//                }
            dos.writeBytes(END);
        } catch (IOException e) {
            throw e;
    ...
    private static void writeParams(String path, DataOutputStream dos,
        ...
        if (params != null && !params.isEmpty()) {
            ...
            if (BaseConfig.DIGITAL_CHECK) {  //开启数字签名
                String datetime = TimeUtil.getCurrentTime("yyyy-MM-dd HH:mm:ss");
                data.append("&").append("datetime=").append(datetime).append("&");
                String[] tempPath = path.split("/");
                String sign = Md5Util.getMd5(BaseConfig.DATAKEY + "|" + datetime + "|" + tempPath[tempPath.length - 1]);
    ...
    public static String sendPOSTForString(String path,
        ...
        try {
            ...
            conn.setConnectTimeout(BaseConfig.TIMEOUT_CONNECT_HTTP);
            conn.setReadTimeout(BaseConfig.TIMEOUT_READ_HTTP);
            conn.setRequestMethod("POST");
            conn.setDoOutput(true);
            ...
            Logger.d("The responsecode is " + code);
