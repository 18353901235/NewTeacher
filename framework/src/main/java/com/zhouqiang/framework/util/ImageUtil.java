...
public class ImageUtil {
     ...
     *
     * @param path     图片地址目录
     * @param reheight 想要的高度
     * @param rewidth  想要的宽度
     * @param quality  图片清晰度（0--100 100为无失真压缩），若传0--100以外值则为默认值100
     * @param savedir  保存目录
     * @return String
     * @throws IOException
     */
    public static String compressPictureWithSaveDir(String path, int reheight,
        ...
        bitmap.recycle();
        System.gc();
        Logger.d("The new picture's path is " + savepath);
        return savepath;
    }
    ...
    public static String compressPictureDepthWithSaveDir(String path, int reheight,
        ...
        while (bos.toByteArray().length / 1024 > BaseConfig.MAX_IMAGE_SIZE) {
            bos.reset();// 置为空
            // 每次都减少10
            options -= 10;
            // 压缩options%
    ...
    public static Bitmap getLocPicByWidthorHeight(String path, int want,
        ...
        Logger.d("The picture path is " + path);
        Logger.d("The picture inSampleSize is " + inSampleSize);
        Logger.d("The picture scale is " + scale);
        Logger.d("The picture degree is " + getPictureDegree(path));
     ...
     *
     * @param path       图片路径
     * @param wantHeight 想要的高度
     * @param wantWidth  想要的宽度
     * @return Bitmap
     * @throws IOException
     */
    ...
    public static Bitmap getLocalPicture(String path, int wantHeight,
        ...
        try {
            ...
            Logger.d("The picture path is " + path);
            Logger.d("The picture inSampleSize is " + inSampleSize);
            Logger.d("The picture scale is " + scale_w + "-----"
                    + scale_h);
            Logger
                    .d("The picture degree is " + getPictureDegree(path));
            Bitmap bit = Bitmap.createBitmap(bitmap, 0, 0, options.outWidth,
                    options.outHeight, null, true);
            return bit;
        } catch (Exception e) {
            return null;
        }
